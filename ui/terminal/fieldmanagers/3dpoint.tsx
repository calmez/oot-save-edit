...
import { FormFieldManagerWithSubfieldsFactory, SubfieldProps } from "./subfield.tsx";
...
export const Point3DFormFieldManager = FormFieldManagerWithSubfieldsFactory<
  FormFieldPoint3D,
  FaroresWindWarp
>("point3d", (props: SpecificFormFieldRendererProps<FormFieldPoint3D & {
  subfields: Record<keyof FaroresWindWarp, SubfieldProps<FormFieldPoint3D, FaroresWindWarp>>;
}>) => (
    <>
      {"[" +
        Object.keys(props.value ?? { x: 0, y: 0, z: 0, yRotation: 0 })
          .map((key: string) => {
            const label = props.field.subfields[key as keyof FaroresWindWarp].props.label;
